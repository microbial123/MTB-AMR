...
# 设置 Apple Accelerate Framework 的线程数
...
def ML_run(
    logger, bacteria, antb, validation_no, file_name, To_predict, supplementary_fig_no
):
    ...
    for train_index, test_index in tqdm(skf.split(X, Y), desc="StratifiedKFold"):
        ...
        for name, model in tqdm(models, desc="myDF3"):
            ...
            myDF3 = myDF3._append(
                {"classifier": name, f"ten_f_CV{i + 1}": mean}, ignore_index=True
            )
        ...
        for name, model in tqdm(models, desc="myDF12"):
            ...
            logger.info(
                "Train model: {} done. Then start predicting X_train...".format(name)
            )
            Y_train_pred = model.predict(X_train)
            Y_test_pred = model.predict(X_test)
            logger.info("{} predict done.".format(name))
            Tr_precision = precision_score(
                Y_train, Y_train_pred, average="macro"
            )
            Tr_recall = recall_score(Y_train, Y_train_pred, average="macro")
            Tr_f1 = f1_score(Y_train, Y_train_pred, average="macro")
            logger.info(
                "Model {} X_train: Tr_precision:{},\t Tr_recall:{},\t Tr_f1:{}.".format(
                    name, Tr_precision, Tr_recall, Tr_f1
                )
            )
            myDF1 = myDF1._append(
                {
                    "classifier": name,
                    f"tr_precision{i + 1}": Tr_precision,
                    f"tr_recall{i + 1}": Tr_recall,
                    f"tr_f1 {i + 1}": Tr_f1,
                },
                ignore_index=True,
            )
            # report = sklearn.metrics.classification_report(Y_test, Y_test_pred)
            Te_precision = precision_score(Y_test, Y_test_pred, average="macro")
            Te_recall = recall_score(Y_test, Y_test_pred, average="macro")
            Te_f1 = f1_score(Y_test, Y_test_pred, average="macro")
            logger.info(
                "Model {} X_train: Te_precision:{},\t Te_recall:{},\t Te_f1:{}.".format(
                    name, Te_precision, Te_recall, Te_f1
                )
            )
            myDF2 = myDF2._append(
                {
                    "classifier": name,
                    f"te_precision{i + 1}": Te_precision,
                    f"te_recall{i + 1}": Te_recall,
                    f"te_f1 {i + 1}": Te_f1,
                },
                ignore_index=True,
            )
            ...
            a_u_c = roc_auc_score(Y_test, y_pred_proba)
            logger.info("Model {} AU_ROC: fpr:{},\t tpr:{}.".format(name, fpr, tpr))
            myDF4 = myDF4._append(
                {"a classifier": name, f"au ROC {i + 1}": a_u_c}, ignore_index=True
            )
            ...
            logger.info(
                "Predict: {} done. calculate precision-recall curve...".format(name)
            )
            precision, recall, _ = precision_recall_curve(Y_test, y_pred_proba5)
            logger.info(
                "Predict: {} done. calculate au precision-recall curve...".format(name)
            )
            ...
            logger.info(
                "Model {} AU_ROC: precision:{},\t recall:{},\t area:{},\t f1:{}.".format(
                    name, precision, recall, area, f1
                )
            )
            myDF5 = myDF5._append(
                {"a classifier": name, f"au PR {i + 1}": area}, ignore_index=True
            )
        ...
        Area_Under_Precision_Recall = pd.concat(
            [Area_Under_Precision_Recall, myDF5], axis=1
        )
        i += 1

    Tf_CV.to_csv(
        f"{bacteria}_{antb}_All_Set_Tf_CV_{validation_no}-fold_CV.csv", encoding="utf-8"
    )
    Training_Performance.to_csv(
        f"{bacteria}_{antb}_All_Set_Performance_Training_Performance_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    Test_Performance.to_csv(
        f"{bacteria}_{antb}_All_Set_Test_Training_Performance_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    Area_Under_ROC.to_csv(
        f"{bacteria}_{antb}_All_Set_Test_Area_Under_ROC_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    Area_Under_Precision_Recall.to_csv(
        f"{bacteria}_{antb}_All_Set_Test_Area_Under_Precision_Recall_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    ...
    # training f1 average Training_Performance.filter(like='tr_precision').mean(axis=1)
    tr_f1_avg = Training_Performance.filter(like="tr_f1").mean(axis=1)
    tr_f1_avg = tr_f1_avg.rename("tr_f1_avg", inplace=True)
    # Training precision average
    tr_precision_avg = (
        Training_Performance.filter(like="tr_precision").mean(axis=1)
    )
    tr_precision_avg = tr_precision_avg.rename("tr_precision_avg", inplace=True)

    # Training recall average
    tr_recall_avg = Training_Performance.filter(like="tr_recall").mean(axis=1)
    tr_recall_avg = tr_recall_avg.rename("tr_recall_avg", inplace=True)

    # Test f1 average
    te_f1_avg = Test_Performance.filter(like="te_f1").mean(axis=1)
    te_f1_avg = te_f1_avg.rename("te_f1_avg", inplace=True)

    # Test precision average
    te_precision_avg = (
        Test_Performance.filter(like="te_precision").mean(axis=1)
    )
    te_precision_avg = te_precision_avg.rename("te_precision_avg", inplace=True)

    # Test recall average
    te_recall_avg = Test_Performance.filter(like="te_recall").mean(axis=1)
    te_recall_avg = te_recall_avg.rename("te_recall_avg", inplace=True)

    # Tf_CV average
    Tf_CV_Avg = Tf_CV.filter(like="ten_f_CV").mean(axis=1)
    Tf_CV_Avg = Tf_CV_Avg.rename("Tf_CV_Avg", inplace=True)

    # Area_Under_ROC average
    au_ROC_avg = Area_Under_ROC.filter(like="au ROC").mean(axis=1)
    au_ROC_avg = au_ROC_avg.rename("au_ROC_avg", inplace=True)

    # Area_Under_Precision_Recall average
    au_PR_avg = Area_Under_Precision_Recall.filter(like="au PR").mean(axis=1)
    ...
    for name, model in models:
        ...
        # fit model
        scores = (
            cross_val_score(model, X, Y, cv=cv, scoring="accuracy", n_jobs=2)
            .mean()
    ...
    Final_All_set_Results.to_csv(
        f"{bacteria}_{antb}_All_Set_Performance_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    ...
    for train_index, test_index in tqdm(skf.split(X, Y), desc="StratifiedKFold"):
        ...
        feature_importancek = pd.DataFrame(
            list_of_tuplesk, columns=[f"Feature fold{j}", f"Importance fold{j}"]
        )

        # Sort the dataframe, descending
        logger.info("Sort the dataframe Feature fold {}...".format(j))
        feature_importance_sortedk = feature_importancek.sort_values(
            by=f"Importance fold{j}", ascending=False
        )

        # Remove the square brackets from the dataframe
        feature_importance_sortedk[f"Feature fold{j}"] = feature_importance_sortedk[
            f"Feature fold{j}"
        ].str.get(0)
        feature_importance_sortedk = feature_importance_sortedk
    ...
    cdf1 = Top_consistent[["Feature fold0"]].rename(
        columns={"Feature fold0": "Feature"}
    )
    cdf2 = Top_consistent[["Feature fold1"]].rename(
        columns={"Feature fold1": "Feature"}
    )
    cdf3 = Top_consistent[["Feature fold2"]].rename(
        columns={"Feature fold2": "Feature"}
    )
    cdf4 = Top_consistent[["Feature fold3"]].rename(
        columns={"Feature fold3": "Feature"}
    )
    cdf5 = Top_consistent[["Feature fold4"]].rename(
        columns={"Feature fold4": "Feature"}
    )
    cdf6 = Top_consistent[["Feature fold5"]].rename(
        columns={"Feature fold5": "Feature"}
    )
    ...
    Final_Consistent_Genes_per_fold.to_csv(
        f"{bacteria}_{antb}_Consistent_Genes_Per_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    ...
    open_consistent_genes = pd.read_csv(
        f"{bacteria}_{antb}_Consistent_Genes_Per_{validation_no}-fold_CV.csv"
    )
    ...
    target_genesTT = open_consistent_genes[["Feature"]].rename(
        columns={"Feature": "Consistent genes"}
    )
    ...
    target_genes_good = target_genesTT._append(
        {"Consistent genes": f"{antb}"}, ignore_index=True
    )
    ...
    target_genes_good1 = target_genesTT._append(
        {"Consistent genes": "phenotype"}, ignore_index=True
    )
    ...
    Tf_CV.to_csv(
        f"{bacteria}_{antb}_Intersect_Set_Tf_CV_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    Training_Performance.to_csv(
        f"{bacteria}_{antb}_Intersect_Set_Performance_Training_Performance_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    Test_Performance.to_csv(
        f"{bacteria}_{antb}_Intersect_Set_Performance_Test_Performance_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    Area_Under_ROC.to_csv(
        f"{bacteria}_{antb}_Intersect_Set_Performance_Area_Under_ROC_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    Area_Under_Precision_Recall.to_csv(
        f"{bacteria}_{antb}_Intersect_Set_Performance_Area_Under_Precision_Recall_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    ...
    Intersection_set_Results.to_csv(
        f"{bacteria}_{antb}_Intersection_Set_Performance_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    ...
    for i in range(10):
        ...
        for train_index, test_index in tqdm(skf.split(X, Y), desc="StratifiedKFold"):
            ...
            for name, model in tqdm(models, desc="myDF"):
                ...
                std = results.std()
                logger.info(
                    "Model {} 10fold: mean:{},\t std:{}.".format(name, mean, std)
                )
                myDF = myDF._append(
                    {"classifier": name, f"ten_f_CV{ij + 1}": mean}, ignore_index=True
                )
            ...
            for name, model in tqdm(models, desc="myDF1"):
                ...
                model = model.fit(X_train, Y_train)
                logger.info(
                    "Train model: {} done. Then start predicting X_train...".format(
                        name
                    )
                )
                Y_train_pred = model.predict(X_train)
                ...
                myDF1 = myDF1._append(
                    {
                        "classifier": name,
                        f"tr_precision{ij + 1}": Tr_precision,
                        f"tr_recall{ij + 1}": Tr_recall,
                        f"tr_f1 {ij + 1}": Tr_f1,
                    },
                    ignore_index=True,
                )
                ...
                myDF2 = myDF2._append(
                    {
                        "classifier": name,
                        f"te_precision{ij + 1}": Te_precision,
                        f"te_recall{ij + 1}": Te_recall,
                        f"te_f1 {ij + 1}": Te_f1,
                    },
                    ignore_index=True,
                )
                ...
                logger.info("Model {} AU_ROC: fpr:{},\t tpr:{}.".format(name, fpr, tpr))
                a_u_c = roc_auc_score(Y_test, y_pred_proba3)
                myDF3 = myDF3._append(
                    {"classifier": name, f"au ROC {ij + 1}": a_u_c}, ignore_index=True
                )
                logger.info("Start predict probabilities {}...".format(name))
                ...
                # calculate au precision-recall curve
                logger.info(
                    "Predict: {} done. calculate au precision-recall curve...".format(
                        name
                    )
                )
                area = auc(recall, precision)
                ...
                myDF4 = myDF4._append(
                    {"classifier": name, f"au PR {ij + 1}": area}, ignore_index=True
                )
            ...
            Area_Under_Precision_Recall = pd.concat(
                [Area_Under_Precision_Recall, myDF4], axis=1
            )
            ij += 1
            # List for output
        myDF5 = pd.DataFrame(columns=[])
        ...
        for name, model in models:
            ...
            myDF5 = myDF5._append(
                {"classifier": name, f"Loo_CV {i + 1}": scores}, ignore_index=True
            )
        Loo_CV = pd.concat([Loo_CV, myDF5], axis=1)
        Loo_CV.to_csv(
            f"{bacteria}_{antb}_Random_Set_Test_Loo_CV_{validation_no}-fold_CV.csv",
            encoding="utf-8",
        )

        Tf_CV.to_csv(
            f"{bacteria}_{antb}_Random_Set_Performance_Tf_CV_{validation_no}-fold_CV.csv",
            encoding="utf-8",
        )
        Training_Performance.to_csv(
            f"{bacteria}_{antb}_Random_Set_Performance_Training_Performance_{validation_no}-fold_CV.csv",
            encoding="utf-8",
        )
        Test_Performance.to_csv(
            f"{bacteria}_{antb}_Random_Set_Test_Training_Performance_{validation_no}-fold_CV.csv",
            encoding="utf-8",
        )
        Area_Under_ROC.to_csv(
            f"{bacteria}_{antb}_Random_Set_Test_Area_Under_ROC_{validation_no}-fold_CV.csv",
            encoding="utf-8",
        )
        Area_Under_Precision_Recall.to_csv(
            f"{bacteria}_{antb}_Random_Set_Test_Area_Under_Precision_Recall_{validation_no}-fold_CV.csv",
            encoding="utf-8",
        )
        ...
        tr_recall_avg = (
            Training_Performance.filter(like="tr_recall").mean(axis=1)
        )
        tr_recall_avg = tr_recall_avg.rename("tr_recall_avg", inplace=True)
        ...
        Loo_CV_Avg = Loo_CV.filter(like="Loo_CV").mean(axis=1)
        ...
        # Area_Under_Precision_Recall average
        au_PR_avg = (
            Area_Under_Precision_Recall.filter(like="au PR").mean(axis=1)
        )
        au_PR_avg = au_PR_avg.rename("au_PR_avg", inplace=True)
    ...
    Random_Set_Results = pd.concat(
        [Models, tr_pa, tr_ra, tr_fa, te_pa, te_ra, te_fa, Tf_Ca, Loo_Ca, au_Ra, au_Pa],
        axis=1,
    )
    ...
    Random_Set_Results.to_csv(
        f"{bacteria}_{antb}_Random_Set_Performance_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    ...
    df6.to_csv(
        f"{bacteria}_{antb}_F1_comparision_{validation_no}-fold_CV.csv",
        encoding="utf-8",
    )
    ...
    fig, axes = plt.subplots(
        nrows=3, ncols=3, sharex=False, sharey=False, figsize=(15, 10)
    )
    ...
    fig.savefig(
        f"ML_Plot_{bacteria}_{antb}_{validation_no}-fold_CV.tiff",
        dpi=300,
        format="tiff",
...
@click.command("建模及性能测试(结果在运行目录生成)")
@click.option("-a", "--antb", type=str, help="药物名称")
@click.option("-l", "--fig_lable", type=str, help="Fig编号标签")
@click.option(
    "-r", "--training_data", type=click.Path(exists=True), help="训练数据，csv格式"
)
@click.option(
    "-t", "--test_data", type=click.Path(exists=True), help="测试数据，csv格式"
)
def main(antb, fig_lable, training_data, test_data):
    # Dictionary of antibiotics
    antb_SN = {antb: fig_lable}
    ...
    for antb, SN in antb_SN.items():
        ...
        f_handler = logging.FileHandler(
            "{}_{}.txt".format(
                antb, time.strftime("%Y-%m-%d%H-%M-%S", time.localtime())
            )
        )
